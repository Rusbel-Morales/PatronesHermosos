...
import { frasesMotivacionales } from "../../assets/frasesMotivacionales.js";
...
const InicioSesion = () => {
  ...
  setIsSubmitting(true);
    ...
    try {
      const { nombre, token, rol, sedeData} = await postLogin(email, password);
      if (token) {
        setUsername(nombre);
        setToken(token); // Guarda el token en el estado
        localStorage.setItem("token", token); // Guarda el token en localStorage
        localStorage.setItem("username", nombre); // Guarda el nombre en localStorage
        localStorage.setItem("rol", rol); // Guarda el rol en localStorage
        if (rol === "coord_sede" && sedeData) {
          localStorage.setItem("id_sede", sedeData.id_sede);
          localStorage.setItem("nombre_sede", sedeData.nombre_sede);
          localStorage.setItem("fecha_inicio", sedeData.fecha_inicio);
        }
        ...
        setTimeout(() => {
          if (rol === "coord_general") {
            navigate("/coord-general/dashboard");
          } else if (rol === "coord_sede") {
            navigate("/coord-sede/dashboard");
          }
        }, 100);
      } else {
        setError();
