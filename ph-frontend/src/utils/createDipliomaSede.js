 ...
 * @param {string} data.rol_preferencia - rol_preferencia del usuario
...
function determineDiplomaRoute(rol_preferencia, participante) {
    const pdfPath = {
    ...
    return participante ? pdfPath["participante"] : pdfPath[rol_preferencia];
}

export async function modifyPdf(data, campus, fecha, participante) {
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const diplomaPath = determineDiplomaRoute(data[i].rol_preferencia, participante);
            ...
            const dateInSpanish = convertDateToSpanish(fecha);
